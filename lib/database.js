...
module.exports = {
  ...
  create_cg: function(coin, cb) {
    var newCg = new Cg({
      symbol: coin,
      rank: "-1",
    });

    newCg.save(function(err) {
      if (err) {
        console.log(err);
        return cb();
      } else {
        console.log("initial CoinGecko entry created for %s", coin);
        return cb();
      }
    })
  },

  check_cg: function(coin, cb) {
    Cg.findOne({symbol: coin}, function(err, exists) {
      if(exists) {
        return cb(true);
      } else {
        return cb(false);
  ...
  get_cg: function(coin, cb) {
    Cg.findOne({symbol: coin}, function(err, data) {
      if(data) {
        return cb(data);
      } else {
        return cb(null);
  ...
  get_distribution: function(richlist, stats, cb){
    ...
    lib.syncLoop(richlist.balance.length, function (loop) {
      ...
      if (i != 0){
        ...
        distribution.t_ignore.percent = percentage;
        distribution.t_ignore.total = (richlist.balance[i].balance / 100000000);
      }
      loop.next();
    }, function(){
      ...
      distribution.t_101plus.total = parseFloat(distribution.supply - distribution.t_ignore.total - distribution.t_76_100.total - distribution.t_51_75.total - distribution.t_26_50.total - distribution.t_1_25.total).toFixed(8);
  ...
  /*update_coingecko_db: function(coin, cb) {
    coingecko.get_data(coin, function(err, obj) {
      if (err !== null) {
        return cb(err);
      }
      Cg.updateMany( { symbol:coin },
	{
          rank: obj.coingecko_rank,
	  volume_24h_usd: obj.market_data.total_volume.usd,
          price_usd: obj.market_data.current_price.usd,
          price_btc: obj.market_data.current_price.btc,
          market_cap_usd: obj.market_data.market_cap.usd,
          available_supply: obj.market_data.circulating_supply,
          total_supply: obj.total_supply,
          percent_change_1h: obj.market_data.price_change_percentage_24h,
          percent_change_24h: obj.market_data.price_change_percentage_24h,
          percent_change_7d: obj.market_data.price_change_percentage_7d,
          last_updated: obj.last_updated,
        }, function() {
          	Stats.updateOne(
	    	{coin:settings.coin},
            	{last_price: obj.market_data.current_price.btc},
...
update_coingecko_db: function(coin, cb) {
      coingecko.get_data(coin, function(err, obj) {
          ...
          Cmc.update({ symbol: coin }, {
              rank: obj ? obj.rank : 0,
              price_usd:obj ?  obj.price_usd : 0,
              price_btc: obj ? obj.price_btc : 0,
              volume_24h_usd: obj ? obj['24h_volume_usd'] : 0,
              market_cap_usd: obj ? obj.market_cap_usd : 0,
              available_supply: obj ? obj.available_supply : 0,
              total_supply: obj ? obj.total_supply : 0,
              percent_change_1h: obj ? obj.percent_change_1h : 0,
              percent_change_24h: obj ? obj.percent_change_24h : 0,
              percent_change_7d: obj ? obj.percent_change_7d : 0,
              last_updated: obj ? obj.last_updated : 0
          }, function() {
              Stats.update({coin:settings.coin}, {
                  last_price: obj ? obj.price_btc : 0,
              }, function(){
                  return cb(null);
              });
          }, function(err) {
