...
function get_ticker(coin, cb) {
    var req_url = base_url + 'coins/' + coin;
    request({ uri: req_url, json: true }, function (error, response, body) {
        if (body.error) {
            ...
            var tickers = {
                rank: 0,
                price_usd: body.tickers[0].converted_last.usd.toString(),
                price_btc: body.tickers[0].converted_last.btc.toString(),
                '24h_volume_usd': body.tickers[0].volume.toString(),
